...
fn output_width() -> usize {
    if let Ok(cols) = std::env::var("COLS") {
        ...
        // Invalid or zero COLS: fall through to default
    }
...
mod tests {
    ...
    fn test_too_many_addresses() {
        ...
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("input.sed:3: command q expects up to 1 address(es), found 2")
        );
    }
    ...
    fn test_invalid_address_range_posix() {
        ...
        let msg = result.unwrap_err().to_string();
        assert!(
            msg.contains("input.sed:1: command i expects up to 1 address(es), found 2")
        );
    }
